...
router.patch('/me', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'UPDATE users SET username=$1 WHERE id=$2 RETURNING id, email, username, avatar_url, is_premium',
      [username, req.userId]
    );
    res.json(result.rows[0]);
  } catch (err) {
  ...
  try {
    await pool.query(
      \`INSERT INTO user_preferences (user_id, genres, artist_names, completed)
       VALUES ($1, $2, $3, true)
       ON CONFLICT (user_id)
       DO UPDATE SET genres = $2, artist_names = $3, completed = true\`,
      [req.userId, genres || [], artist_names || []]
    );
    res.json({ ok: true });
  } catch (err) {
...
router.get('/preferences', authMiddleware, async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT completed FROM user_preferences WHERE user_id = $1',
      [req.userId]
    );
...
fs.writeFileSync('backend/routes/users.js', content);
console.log('users.js written successfully!');
